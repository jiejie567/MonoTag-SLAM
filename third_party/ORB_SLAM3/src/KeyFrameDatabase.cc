...
vector<KeyFrame*> KeyFrameDatabase::DetectRelocalizationCandidates(Frame *F, Map* pMap)
{
    ...
    for(list<pair<float,KeyFrame*> >::iterator it=lScoreAndMatch.begin(), itend=lScoreAndMatch.end(); it!=itend; it++)
    {
        ...
        {
            KeyFrame* pKF2 = *vit;
            // A current query ID only means this neighbor shared a word.
            // Scores are computed only above minCommonWords; otherwise the
            // stored value belongs to an older query or is uninitialized.
