...
namespace ORB_SLAM3
{
// Shared relative information for checked, subpixel fixed-tag corners.
// Weak observations retain their 0.25 multiplier; the Huber transition is
// expressed in pixels by scaling its whitened threshold with sqrt(info).
static constexpr double kTagCornerInformation = 64.0;

class EssentialGraphTagProjectionEdge final
    : public g2o::BaseUnaryEdge<2, Eigen::Vector2d, g2o::VertexSim3Expmap>
{
public:
    ...
    void computeError() override
    {
        ...
        const Eigen::Vector3d cameraPoint = vertex->estimate().map(worldPoint);
        if(!camera || !cameraPoint.allFinite() || cameraPoint.z() <= 1e-9)
        {
            _error.setConstant(1e6);
            return;
        }
        _error = _measurement - camera->project(cameraPoint);
    }
...
static double FixedTagReprojectionRms(
    KeyFrame* keyframe, const g2o::SE3Quat& cameraFromWorld);

static void AddFixedTagProjectionEdges(
    g2o::SparseOptimizer &optimizer,
    KeyFrame* pKF,
    const float huberDelta,
    const bool requireLocalConsistency=false)
{
    // Marker corners are expressed in metres. Before metricization an ORB
    // map still has an arbitrary gauge, so mixing the two coordinate systems
    // inside ordinary BA is invalid and can make the normal equations
    // singular. Tracking accumulates those observations as scale evidence;
    // projection factors become active only after an atomic metric commit.
    if(!pKF || !pKF->GetMap() || !pKF->GetMap()->mbMetric ||
       !pKF->mbTagObservationActive ||
       pKF->mvTagWorldPoints.size() != pKF->mvTagImagePoints.size() ||
       (!pKF->mvTagPointWeights.empty() &&
        pKF->mvTagPointWeights.size() != pKF->mvTagWorldPoints.size()))
        return;
    g2o::OptimizableGraph::Vertex* vertex =
        dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKF->mnId));
    if(!vertex)
        return;
    if(requireLocalConsistency) {
        const Sophus::SE3f current=pKF->GetPose();
        const g2o::SE3Quat currentEstimate(
            current.unit_quaternion().cast<double>(),
            current.translation().cast<double>());
        const double priorResidual=FixedTagReprojectionRms(pKF,currentEstimate);
        // A large residual is an interval re-anchoring/loop candidate, not a
        // local perturbation. Pulling one local window onto it creates a pose
        // discontinuity while the older path remains in the previous gauge.
        if(!std::isfinite(priorResidual) || priorResidual>100.0)
            return;
    }
    const double informationWeight =
        kTagCornerInformation * std::max(0.35f, pKF->mTagObservationConfidence);
    for(size_t index = 0; index < pKF->mvTagWorldPoints.size(); ++index)
    {
        EdgeSE3ProjectXYZOnlyPose* edge = new EdgeSE3ProjectXYZOnlyPose();
        edge->setVertex(0, vertex);
        edge->setMeasurement(Eigen::Vector2d(
            pKF->mvTagImagePoints[index].x,
            pKF->mvTagImagePoints[index].y));
        const float pointWeight = pKF->mvTagPointWeights.empty()
            ? 1.0f : pKF->mvTagPointWeights[index];
        edge->setInformation(Eigen::Matrix2d::Identity() * informationWeight * pointWeight);
        edge->Xw = pKF->mvTagWorldPoints[index].cast<double>();
        edge->pCamera = pKF->mpCamera;
        g2o::RobustKernelHuber* robust = new g2o::RobustKernelHuber;
        edge->setRobustKernel(robust);
        robust->setDelta(huberDelta * std::sqrt(informationWeight * pointWeight));
...
static double FixedTagReprojectionRms(
    KeyFrame* keyframe, const g2o::SE3Quat& cameraFromWorld)
{
    if(!keyframe || !keyframe->mbTagObservationActive || !keyframe->mpCamera ||
    ...
    double squared=0.0, information=0.0;
    for(size_t index=0;index<keyframe->mvTagWorldPoints.size();++index) {
        const double weight=keyframe->mvTagPointWeights.empty()
            ? 1.0 : keyframe->mvTagPointWeights[index];
        if(!std::isfinite(weight) || weight<=0.0) continue;
        const Eigen::Vector3d cameraPoint=
            cameraFromWorld.map(keyframe->mvTagWorldPoints[index].cast<double>());
        if(!cameraPoint.allFinite() || cameraPoint.z()<=1e-6)
            return std::numeric_limits<double>::infinity();
        const Eigen::Vector3f cameraPointFloat=cameraPoint.cast<float>();
        const Eigen::Vector2f projected=keyframe->mpCamera->project(cameraPointFloat);
        const Eigen::Vector2d residual=projected.cast<double>()-
            Eigen::Vector2d(keyframe->mvTagImagePoints[index].x,
                            keyframe->mvTagImagePoints[index].y);
        squared+=weight*residual.squaredNorm();
        information+=weight;
    }
    return information>0.0 ? std::sqrt(squared/information)
                                 ...
                                 int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    ...
    for(KeyFrame* pKF : vpKFs)
        if(!pKF->isBad())
            AddFixedTagProjectionEdges(optimizer, pKF, thHuber2D);
...
int Optimizer::PoseOptimization(Frame *pFrame,
    const std::vector<Eigen::Vector3f> &tagWorldPoints,
    const std::vector<cv::Point2f> &tagImagePoints,
    const std::vector<float> &tagWeights)
{
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolver_6_3::LinearSolverType * linearSolver;
    ...
    vSE3->setFixed(false);
    optimizer.addVertex(vSE3);
    bool hasTagEdges = false;
    if(tagWorldPoints.size()==tagImagePoints.size() &&
       (tagWeights.empty() || tagWeights.size()==tagWorldPoints.size())) {
        for(size_t i=0;i<tagWorldPoints.size();++i) {
            auto* edge=new EdgeSE3ProjectXYZOnlyPose();
            edge->setVertex(0,vSE3);
            edge->setMeasurement(Eigen::Vector2d(tagImagePoints[i].x,tagImagePoints[i].y));
            const double information = kTagCornerInformation*(tagWeights.empty()?1.0:tagWeights[i]);
            edge->setInformation(Eigen::Matrix2d::Identity()*information);
            edge->Xw=tagWorldPoints[i].cast<double>();
            edge->pCamera=pFrame->mpCamera;
            auto* huber=new g2o::RobustKernelHuber();
            huber->setDelta(std::sqrt(5.991 * information));
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges)
{
    ...
    lLocalMapPoints.sort([](MapPoint* first,MapPoint* second) {
    ...
    // The original marker-only keyframe has no ORB points, so it need not
    // be covisible with this local window. Two reliable metric tag camera
    // poses with a baseline constrain its gauge AND scale without freezing
    // an unrelated visual keyframe. One tagged view alone leaves scale free.
    bool metricTagGauge = false;
    std::vector<Eigen::Vector3f> tagCenters;
    if(pCurrentMap->mbMetric) {
        for(KeyFrame* keyframe : lLocalKeyFrames) {
            if(!keyframe->mbTagObservationActive || keyframe->mTagObservationConfidence<0.35f ||
               ...
               (!keyframe->mvTagPointWeights.empty() &&
                ...
                continue;
            size_t strong=0;
            for(size_t i=0;i<keyframe->mvTagWorldPoints.size();++i)
                if(keyframe->mvTagPointWeights.empty() ||
                   (i<keyframe->mvTagPointWeights.size() && keyframe->mvTagPointWeights[i]>=0.99f))
                    ++strong;
            if(strong<4) continue;
            const auto center=keyframe->GetCameraCenter();
            for(const auto& previous:tagCenters)
                if((center-previous).norm()>=0.01f) metricTagGauge=true;
            tagCenters.push_back(center);
        }
    }
    if(num_fixedKF == 0 && !metricTagGauge)
    {
    ...
    for(KeyFrame* pKFi : lLocalKeyFrames)
        AddFixedTagProjectionEdges(optimizer, pKFi, thHuberMono, true);
    const size_t localTagProjectionFactors = optimizer.edges().size();
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        const map<KeyFrame*,tuple<int,int>> observations = pMP->GetObservations();
        vector<pair<KeyFrame*,tuple<int,int>>> orderedObservations(
            observations.begin(),observations.end());
        sort(orderedObservations.begin(),orderedObservations.end(),
             [](const pair<KeyFrame*,tuple<int,int>>& first,
                const pair<KeyFrame*,tuple<int,int>>& second) {
                 return first.first->mnId<second.first->mnId;
             });

        //Set edges
        for(const auto& observation:orderedObservations)
        {
    ...
    if(pCurrentMap->mbMetric && !pCurrentMap->IsInertial()) {
        std::vector<double> ratios;
        auto* camera=static_cast<g2o::VertexSE3Expmap*>(optimizer.vertex(pKF->mnId));
        const Sophus::SE3f original=pKF->GetPose();
        for(size_t i=0;i<vpEdgesMono.size();++i) {
            if(vpEdgeKFMono[i]!=pKF) continue;
            MapPoint* point=vpMapPointEdgeMono[i];
            auto* vertex=static_cast<g2o::VertexSBAPointXYZ*>(
                optimizer.vertex(point->mnId+maxKFid+1));
            const Eigen::Vector3f originalPoint=original*point->GetWorldPos();
            const double before=originalPoint.z();
            if(before<=0) continue;
            const Eigen::Vector2d error=pKF->mpCamera->project(originalPoint).cast<double>()-
                vpEdgesMono[i]->measurement();
            // Only already-consistent observations can veto a new proposal.
            if(error.dot(vpEdgesMono[i]->information()*error)>5.991) continue;
            const double after=camera->estimate().map(vertex->estimate()).z();
            ratios.push_back(after/before);
        }
        double median=1.0;
        const bool accepted=LocalMetricDepthChangeAccepted(ratios,median);
        if(std::getenv("ORB_SLAM3_LOCAL_SCALE_AUDIT") || !accepted)
          cout << "LOCAL_SCALE_AUDIT kf=" << pKF->mnId << " t=" << pKF->mTimeStamp
             << " fixed=" << num_fixedKF << " local=" << lLocalKeyFrames.size()
             << " tags=" << tagCenters.size()
             << " tag_factors=" << localTagProjectionFactors << " samples=" << ratios.size()
             << " ratio=" << median << " accepted=" << accepted << endl;
        // No observation erasure, pose or point update has happened yet.
        if(!accepted) return;
    }
    ...
    // Local BA is staged in g2o until this point.  Do not publish an ORB-only
    // deformation that contradicts a previously admitted metric marker
    // observation.  This catches planar/gauge compromises that can retain a
    // low aggregate background error while moving a tagged keyframe by
    // decimetres and later making high-rate frame reconstruction discontinuous.
    if(pCurrentMap->mbMetric) {
        for(KeyFrame* tagged : lLocalKeyFrames) {
            if(!tagged->mbTagObservationActive || tagged->mvTagWorldPoints.size()<4)
                continue;
            auto* vertex=static_cast<g2o::VertexSE3Expmap*>(optimizer.vertex(tagged->mnId));
            if(!vertex) continue;
            const Sophus::SE3f original=tagged->GetPose();
            const g2o::SE3Quat originalEstimate(
                original.unit_quaternion().cast<double>(),
                original.translation().cast<double>());
            const double before=FixedTagReprojectionRms(tagged,originalEstimate);
            const double after=FixedTagReprojectionRms(tagged,vertex->estimate());
            const double allowed=std::max(4.0,before+1.0);
            if(std::isfinite(before) && (!std::isfinite(after) || after>allowed)) {
                cout << "LM-LBA rejected: metric marker reprojection "
                     << before << " -> " << after << " px at KF "
                     << tagged->mnId << endl;
                return;
            }
        }
...
{   
    ...
    const bool markerGauge = pMap->HasMetricTagGeometry();
    long fixedGaugeKeyFrameId = long(pMap->GetInitKFid());
    KeyFrame* originKeyFrame = pMap->GetOriginKF();
    if(originKeyFrame && !originKeyFrame->isBad())
        fixedGaugeKeyFrameId = long(originKeyFrame->mnId);
    const bool haveGaugeKeyFrame = std::any_of(vpKFs.begin(), vpKFs.end(),
        [fixedGaugeKeyFrameId](KeyFrame* keyframe) {
            return keyframe && !keyframe->isBad() &&
                long(keyframe->mnId) == fixedGaugeKeyFrameId;
        });
    // Synthetic/preloaded maps may not have a valid origin in their current
    // keyframe set. Only then fall back to the last explicitly validated
    // metric anchor; do not normally move the fixed gauge from A to B after a
    // later scale re-anchoring.
    if(!haveGaugeKeyFrame && markerGauge && pMap->GetMarkerScaleAnchorKFId() >= 0)
    {
        const long candidate = pMap->GetMarkerScaleAnchorKFId();
        const bool available = std::any_of(vpKFs.begin(), vpKFs.end(),
            [candidate](KeyFrame* keyframe) {
                return keyframe && !keyframe->isBad() && long(keyframe->mnId) == candidate;
            });
        if(available)
            fixedGaugeKeyFrameId = candidate;
    }
    ...
    for(size_t i=0, iend=vpKFs.size(); i<iend;i++)
    {
        ...
        // Fix exactly one gauge keyframe. Other marker keyframes remain
        // optimizable and are constrained by their original corner pixels
        // below; freezing all of them preserved stale pose error and prevented
        // a genuinely joint loop correction.
        if(long(pKF->mnId) == fixedGaugeKeyFrameId)
            VSim3->setFixed(true);
    ...
    // Reliable fixed-marker corners are metric unary factors in the same
    // Sim(3) graph as the visual loop. Each stored observation is inserted
    // exactly once. The map's single fixed gauge keyframe removes global
    // freedom; Huber loss prevents one damaged corner from dominating.
    if(markerGauge)
    {
        for(KeyFrame* pKF : vpKFs)
        {
            if(!pKF || pKF->isBad() || !pKF->mbHasTagObservation ||
               !pKF->mbTagObservationActive ||
               !pKF->mpCamera ||
               pKF->mvTagWorldPoints.size() != pKF->mvTagImagePoints.size() ||
               (!pKF->mvTagPointWeights.empty() &&
                pKF->mvTagPointWeights.size() != pKF->mvTagWorldPoints.size()))
                continue;
            auto* vertex = dynamic_cast<g2o::OptimizableGraph::Vertex*>(
                optimizer.vertex(pKF->mnId));
            if(!vertex || vertex->fixed())
                continue;
            const double confidence = std::max(0.35f, pKF->mTagObservationConfidence);
            for(size_t index = 0; index < pKF->mvTagWorldPoints.size(); ++index)
            {
                const double pointWeight = pKF->mvTagPointWeights.empty()
                    ? 1.0 : pKF->mvTagPointWeights[index];
                const cv::Point2f measured = pKF->mvTagImagePoints[index];
                if(!std::isfinite(pointWeight) || pointWeight <= 0.0 ||
                   !pKF->mvTagWorldPoints[index].allFinite() ||
                   !std::isfinite(measured.x) || !std::isfinite(measured.y))
                    continue;
                const double information =
                    kTagCornerInformation * confidence * pointWeight;
                auto* edge = new EssentialGraphTagProjectionEdge();
                edge->setVertex(0, vertex);
                edge->setMeasurement(Eigen::Vector2d(
                    measured.x, measured.y));
                edge->setInformation(Eigen::Matrix2d::Identity() * information);
                edge->worldPoint = pKF->mvTagWorldPoints[index].cast<double>();
                edge->camera = pKF->mpCamera;
                auto* robust = new g2o::RobustKernelHuber();
                robust->setDelta(std::sqrt(5.991 * information));
                edge->setRobustKernel(robust);
                optimizer.addEdge(edge);
            }
        }
    ...
    for(size_t i=0;i<vpKFs.size();i++)
    {
        ...
        pKFi->SetPose(Tiw);
        // This is the single committed scale update for a loop correction.
        // CorrectLoop's provisional poses deliberately do not change the stamp.
        pKFi->mReplayUnitScale /= static_cast<float>(s);
    }
                            ...
                            const bool bFixScale, Eigen::Matrix<double,7,7> &mAcumHessian, const bool bAllPoints)
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        {
            // The Sim3 edge predicts image pixels, not normalized rays.
            // Covisible-only points need a virtual observation in the same
            // pixel units as actual observations of this reference camera.
            obs2 = pKF2->mpCamera->project(P3D2c).cast<double>();
            kpUn2.octave = std::max(0, std::min(pMP2->mnTrackScaleLevel,
                int(pKF2->mvInvLevelSigma2.size())-1));
