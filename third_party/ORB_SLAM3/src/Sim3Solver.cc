    ...
    pCamera1(pKF1->mpCamera), pCamera2(pKF2->mpCamera)
{
    ...
    for(int i1=0; i1<mN1; i1++)
    {
        if(vpMatched12[i1])
        {
            ...
            // Covisibility matches may be observed only by a neighbor of
            // pKF2. Use the supplied observation owner for index/noise lookup;
            // all 3D points still share pKF2's coordinate basis below.
            if(i1 >= vpKeyFrameMatchedMP.size() || !vpKeyFrameMatchedMP[i1])
                continue;
...
void Sim3Solver::ComputeSim3(Eigen::Matrix3f &P1, Eigen::Matrix3f &P2)
{
    ...
    // Horn already returns a quaternion. Converting its zero imaginary part
    // through angle-axis divides by zero for a valid identity rotation.
    Eigen::Quaternionf rotation(evec(0,maxIndex),evec(1,maxIndex),
                                evec(2,maxIndex),evec(3,maxIndex));
    mR12i = rotation.normalized().toRotationMatrix();
...
{
    if(!mT12i.allFinite() || !mT21i.allFinite() || !std::isfinite(ms12i) || ms12i<=0) {
        mnInliersi=0;
        std::fill(mvbInliersi.begin(),mvbInliersi.end(),false);
        return;
    }
