...
namespace ORB_SLAM3
{
    ...
    int ORBmatcher::SearchForInitialization(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<int> &vnMatches12, int windowSize, bool allLevels)
    {
        int nmatches=0;
        ...
        for(size_t i1=0, iend1=F1.mvKeysUn.size(); i1<iend1; i1++)
        {
            ...
            if(level1>0 && !allLevels)
                continue;

            // Metric marker bootstrap has known camera poses and checks the
            // 3D geometry afterwards. It can use all pyramid levels, while
            // ordinary monocular initialization retains its original policy.
            vector<size_t> vIndices2 = F2.GetFeaturesInArea(vbPrevMatched[i1].x,vbPrevMatched[i1].y,
                windowSize, allLevels?std::max(0,level1-1):level1, allLevels?level1+1:level1);
                                           ...
                                           vector<pair<size_t, size_t> > &vMatchedPairs, const bool bOnlyStereo, const bool bCoarse)
    {
            ...
            if(f1it->first == f2it->first)
            {
                for(size_t i1=0, iend1=f1it->second.size(); i1<iend1; i1++)
                {
                    ...
                    for(size_t i2=0, iend2=f2it->second.size(); i2<iend2; i2++)
                    {
                        ...
                        // Existing map points cannot seed another point.
                        if(pMP2)
                            continue;
        ...
        // A feature slot can belong to only one new MapPoint. Resolve collisions
        // after the original geometric/orientation gates so a weak early match
        // cannot hide a better later match. Equal distances keep the first source.
        vector<int> sourceForTarget(pKF2->N,-1);
        vector<int> distanceForTarget(pKF2->N,TH_LOW+1);
        for(size_t i=0; i<vMatches12.size(); ++i)
        {
            const int target = vMatches12[i];
            if(target<0)
                continue;
            const int distance = DescriptorDistance(pKF1->mDescriptors.row(i),
                                                    pKF2->mDescriptors.row(target));
            if(sourceForTarget[target]<0 || distance<distanceForTarget[target])
            {
                if(sourceForTarget[target]>=0)
                    vMatches12[sourceForTarget[target]]=-1;
                sourceForTarget[target]=static_cast<int>(i);
                distanceForTarget[target]=distance;
            }
            else
                vMatches12[i]=-1;
        }
