...
bool Map::HasMetricTagGeometry()
{
    unique_lock<mutex> lock(mMutexMap);
    if(mbMetric || mbMarkerSeed || !mStaticTags.empty())
        return true;
    // Inactive observations also carry fixed-world corners; a later metric
    // alignment must not activate factors silently left in another gauge.
...
{
    unique_lock<mutex> lock(mMutexMap);
    vector<MapPoint*> points(mspMapPoints.begin(),mspMapPoints.end());
    sort(points.begin(),points.end(),[](MapPoint* first,MapPoint* second) {
...
//        delete *sit;
    ...
    mbIMU_BA1 = false;
    mbIMU_BA2 = false;
    mbMetric=false; mbMarkerSeed=false; mbBackgroundReady=false; mbRigidMarkerLayout=false;
    mMetricScale=0; mStaticTags.clear(); mnMarkerScaleAnchorKFId=-1;
    mnMarkerGraphSequence=0; mMarkerInputToWorld=Sophus::SE3f();
}
...
{
    int nMPWithoutObs = 0;
    // EraseObservation can mark a point bad and erase it from mspMapPoints.
    // Iterate a snapshot so cleanup cannot invalidate this traversal.
    const vector<MapPoint*> pointsToClean = GetAllMapPoints();
    for(MapPoint* pMPi : pointsToClean)
    {
        if(!pMPi || pMPi->isBad())
            continue;
