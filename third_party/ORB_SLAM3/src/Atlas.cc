...
void Atlas::clearAtlas()
{
    ...
    mpCurrentMap = static_cast<Map*>(NULL);
    mnLastInitKFidMap = 0;
    mnMarkerGraphSequence=0; mMarkerGraphEvents.clear(); mMarkerMapAliases.clear();
}
...
void Atlas::PreSave()
{
    ...
    // Saving must not mark the live empty active map bad: GetCurrentMap()
    // would then wait forever after a featureless, uninitialized sequence.
    mvpBackupMaps.clear();
    for(Map* map:mspMaps)
        if(map && !map->IsBad() && map->KeyFramesInMap()>0)
            mvpBackupMaps.push_back(map);
    std::sort(mvpBackupMaps.begin(),mvpBackupMaps.end(),
              [](Map* a,Map* b){return a->GetId()<b->GetId();});
    std::set<GeometricCamera*> cameras(mvpCameras.begin(),mvpCameras.end());
    for(Map* map:mvpBackupMaps) map->PreSave(cameras);
}
