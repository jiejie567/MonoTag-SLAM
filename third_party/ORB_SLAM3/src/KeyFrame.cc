...
void KeyFrame::UpdateBestCovisibles()
{
    ...
    sort(vPairs.begin(),vPairs.end(),[](const pair<int,KeyFrame*>& first,
                                       const pair<int,KeyFrame*>& second) {
        if(first.first!=second.first) return first.first<second.first;
        return first.second->mnId>second.second->mnId;
    });
    list<KeyFrame*> lKFs;
    list<int> lWs;
...
void KeyFrame::UpdateConnections(bool upParent)
{
    ...
    for(map<KeyFrame*,int>::iterator mit=KFcounter.begin(), mend=KFcounter.end(); mit!=mend; mit++)
    {
        ...
        if(mit->second>nmax ||
           (mit->second==nmax && (!pKFmax || mit->first->mnId<pKFmax->mnId)))
        {
            nmax=mit->second;
            pKFmax=mit->first;
        }
...
void KeyFrame::SetBadFlag()
{
...

    {
        ...
        if(mpParent){
            ...
            mReplayParentUnitScale = mpParent->mReplayUnitScale;
            mReplayParentMarkerGraph = mpParent->mReplayMarkerGraph;
            mReplayParentMarkerGauge = mpParent->mReplayMarkerGauge;
        }
        mbBad = true;
    }
...
bool KeyFrame::GetReplayReference(Sophus::SE3f &worldFromReference,
                                  float &referenceUnitScale, Map* &map)
{
    vector<KeyFrame*> chain;
    set<KeyFrame*> visited;
    KeyFrame* surviving=this;
    while(surviving && surviving->isBad()) {
        if(!visited.insert(surviving).second) return false;
        chain.push_back(surviving);
        surviving=surviving->GetParent();
    }
    if(!surviving || !(map=surviving->GetMap()) || map->IsBad()) return false;
    worldFromReference=surviving->GetPoseInverse();
    double unitCorrection=1.0;
    KeyFrame* parent=surviving;
    for(auto it=chain.rbegin();it!=chain.rend();++it) {
        KeyFrame* child=*it;
        if(!std::isfinite(child->mReplayParentUnitScale) || child->mReplayParentUnitScale<=0 ||
           !std::isfinite(parent->mReplayUnitScale) || parent->mReplayUnitScale<=0)
            return false;
        unitCorrection *= parent->mReplayUnitScale / child->mReplayParentUnitScale;
        Sophus::SE3f childFromParent=child->mTcp;
        childFromParent.translation() *= unitCorrection;
        worldFromReference=worldFromReference*childFromParent.inverse();
        parent=child;
    }
    referenceUnitScale=static_cast<float>(mReplayUnitScale*unitCorrection);
    return std::isfinite(referenceUnitScale) && referenceUnitScale>0 &&
           worldFromReference.matrix().allFinite();
}

bool KeyFrame::GetReplayMarkerGraph(MarkerGraphTransform& correction)
{
    vector<KeyFrame*> chain;
    set<KeyFrame*> visited;
    KeyFrame* surviving=this;
    ...
    if(!surviving || !surviving->GetMap() || surviving->GetMap()->IsBad()) return false;
    correction=surviving->mReplayMarkerGraph;
    for(auto it=chain.rbegin();it!=chain.rend();++it) {
        KeyFrame* child=*it;
        if(!child->mReplayParentMarkerGraph.finite() || !child->mReplayMarkerGraph.finite()) return false;
        correction=correction*child->mReplayParentMarkerGraph.inverse()*child->mReplayMarkerGraph;
    }
    return correction.finite();
}

bool KeyFrame::GetReplayMarkerGauge(MarkerGraphTransform& correction)
{
    vector<KeyFrame*> chain;
    set<KeyFrame*> visited;
    KeyFrame* surviving=this;
    ...
    if(!surviving || !surviving->GetMap() || surviving->GetMap()->IsBad()) return false;
    correction=surviving->mReplayMarkerGauge;
    for(auto it=chain.rbegin();it!=chain.rend();++it) {
        KeyFrame* child=*it;
        if(!child->mReplayParentMarkerGauge.finite() || !child->mReplayMarkerGauge.finite()) return false;
        correction=correction*child->mReplayParentMarkerGauge.inverse()*child->mReplayMarkerGauge;
    }
    return correction.finite() && std::abs(correction.scale-1.)<1e-6;
}
