...
void MapPoint::EraseObservation(KeyFrame* pKF)
{
    bool bBad=false;
    {
        ...
        if(mObservations.count(pKF))
        {
            ...
            if(mpRefKF==pKF) {
                // The reference sets the depth/octave range for subsequent
                // projection matching. Pointer order changes that range after
                // identical outlier removals in separate process runs.
                const auto next=std::min_element(mObservations.begin(),mObservations.end(),
                    [](const auto& a,const auto& b) {return a.first->mnId<b.first->mnId;});
                mpRefKF=next==mObservations.end()?nullptr:next->first;
            }
