...
public:
    ...
    struct CorrectionRejection {
        unsigned long sequence;
        double timestamp;
        unsigned long keyframeId, mapId, otherMapId;
        std::string kind, reason;
    };
    std::vector<CorrectionRejection> GetCorrectionRejections();
    // Caller holds Atlas::mMutexPoseGraphCorrection.
    void InvalidateBAForMarkerGraph() { CancelGlobalBundleAdjustment(); }

    // Main function
    void Run();

    // Optional post-tracking search. Caller has stopped mapping, loop closing
    // and GBA; candidates use the unchanged native geometric thresholds.
    void RunOfflineLoopSearch();
    ...
    void RunGlobalBundleAdjustment(Map* pActiveMap, unsigned long nLoopKF,
                                   double loopTimestamp, unsigned long correctionGeneration);
...
protected:
    ...
    bool MergeLocal();
    bool MergeLocal2();
    bool RejectUnsupportedCorrection(const std::string &kind, Map* current, Map* other,
                                     unsigned long keyframeId, double timestamp);
    std::mutex mMutexCorrectionRejections;
    std::vector<CorrectionRejection> mvCorrectionRejections;
    ...
    bool mbOfflineLoopSearch = false;
    ...
    // Queue emptiness alone is insufficient: NewDetectCommonRegions pops the
    // current keyframe before place recognition and correction complete.
    ...
    // A cancelled, detached worker is still alive until it returns. Shutdown
    // must wait for all workers, not only the most recently launched one.
    std::set<unsigned long> mGBAGenerations;
    unsigned long RegisterGBAWorkerLocked();
    void FinishGBAWorker(unsigned long generation);
    bool IsGBACommitCurrent(Map* map, bool initialImu, unsigned long generation);
    void CancelGlobalBundleAdjustment();
    void LaunchGlobalBundleAdjustment(Map* map, KeyFrame* keyframe);
    struct GBAWorkerCompletion {
        LoopClosing* owner;
        unsigned long generation;
        ~GBAWorkerCompletion() { owner->FinishGBAWorker(generation); }
    };
