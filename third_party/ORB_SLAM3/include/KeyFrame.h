...
class KeyFrame
{
    ...
    void serialize(Archive& ar, const unsigned int version)
    {
        ...
        ar & mbHasVelocity;
        ar & mbHasTagObservation;
        ar & mbTagObservationActive;
        ar & mTagObservationConfidence;
        ar & mvTagPointWeights;
        ar & mvTagIds;
        ar & mReplayUnitScale;
        size_t tagCount = mvTagWorldPoints.size();
        ar & tagCount;
        if(Archive::is_loading::value) {
            mvTagWorldPoints.resize(tagCount);
            mvTagImagePoints.resize(tagCount);
        }
        for(size_t i=0; i<tagCount; ++i) {
            ar & boost::serialization::make_array(mvTagWorldPoints[i].data(), 3);
            ar & mvTagImagePoints[i].x;
            ar & mvTagImagePoints[i].y;
        }
        if(version >= 1) {
            ar & mReplayMarkerGraph & mReplayParentMarkerGraph;
        }
        if(version >= 2) {
            ar & mReplayMarkerGauge & mReplayParentMarkerGauge;
        }
    }
...
public:
    ...
    KeyFrame* GetParent();
    // Resolve a possibly culled reference in its surviving map and units.
    bool GetReplayReference(Sophus::SE3f &worldFromReference,
...
public:
    ...
    Sophus::SE3f mTcp;
    // Transient: bad KFs are not serialized in Atlas. This stamps the parent's
    // units when mTcp is formed, not this KF's (possibly newer) creation units.
    float mReplayParentUnitScale = 1.0f;
...
public:
    ...
    // Fixed-tag observations live in the metric world frame. They are kept
    // separate from ORB MapPoints so local/global BA can use their corners as
    // absolute reprojection factors without allowing the tag geometry to move.
    bool mbHasTagObservation = false;
    bool mbTagObservationActive = false;
    float mTagObservationConfidence = 0.0f;
    std::vector<Eigen::Vector3f> mvTagWorldPoints;
    std::vector<cv::Point2f> mvTagImagePoints;
    std::vector<float> mvTagPointWeights;
    std::vector<int> mvTagIds;
    float mReplayUnitScale = 1.0f;
    MarkerGraphTransform mReplayMarkerGraph;
    MarkerGraphTransform mReplayParentMarkerGraph;
    bool GetReplayMarkerGraph(MarkerGraphTransform& correction);
    // Fixed marker measurements already use metres. Only an explicit map
    // merge changes their world gauge; local scale/BA corrections do not.
    MarkerGraphTransform mReplayMarkerGauge;
    MarkerGraphTransform mReplayParentMarkerGauge;
    bool GetReplayMarkerGauge(MarkerGraphTransform& correction);
...
BOOST_CLASS_VERSION(ORB_SLAM3::KeyFrame, 2)
