...
/// ORB_SLAM3_PORTABLE_RANDOM=1 selects a fixed Darwin-compatible stream.
/// Set the flag before the first call; it is read once. The portable stream
/// starts at seed 0, matching the offline runner, and explicit seeds retain
/// their uint32 representation. No-argument seeding still uses the clock.
/// Calls share a locked stream: repeatability requires the same call order,
/// not merely the same seed when callers race on different threads.
class Random
{
public:
...
public:
	...
	static T RandomValue(){
		if(PortableRandomEnabled())
			return (T)PortableRandomDraw()/(T)2147483647;
...
private:
  ...
  static bool PortableRandomEnabled();
  static unsigned int PortableRandomDraw();
