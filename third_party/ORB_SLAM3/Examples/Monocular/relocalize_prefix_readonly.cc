...
class GrayVideoReader {
    FILE* pipe=nullptr;cv::Size size;int next=0;
public:
    GrayVideoReader(const std::string& video,const std::string& ffmpeg,cv::Size shape,int last):size(shape) {
        const auto command=shellQuote(ffmpeg)+" -v error -threads 1 -noautorotate -i "+shellQuote(video)+
            " -map 0:v:0 -frames:v "+std::to_string(last+1)+" -threads 1 -f rawvideo -pix_fmt bgr24 -";
        pipe=popen(command.c_str(),"r");if(!pipe)throw std::runtime_error("video_decoder_start_failed");
    }
    ~GrayVideoReader() {if(pipe)pclose(pipe);}
    cv::Mat imageAt(int frame) {
        if(frame<next)throw std::runtime_error("nonsequential_decode_request");
        cv::Mat image(size,CV_8UC3);
        while(next<=frame) {
            const size_t expected=image.total()*image.elemSize();size_t read=0;
            while(read<expected) {
                const size_t n=fread(image.data+read,1,expected-read,pipe);
                if(!n)throw std::runtime_error("video_decode_failed");read+=n;
            }
            ++next;
        }
        cv::cvtColor(image,image,cv::COLOR_BGR2GRAY);return image;
    }
    GrayVideoReader(const GrayVideoReader&)=delete;
};
...
void writeResult(std::ostream& out,const Result& result,long mapId,unsigned long revision,double effective,
                 bool correspondenceMode,bool evidenceMode=false) {
    // Evidence is not a replacement pose. A separate pixel/commit validator
    // must accept a refinement; existing prefix readers still see accepted=false.
    const bool evidence=result.accepted||(evidenceMode&&result.candidate&&result.connected);
    out<<"{\"type\":\""<<(result.supportOnly?"support":"frame")<<"\",\"frame\":"<<result.frame<<",\"timestamp_s\":"<<result.time
       ...
       <<(evidenceMode?"offline-final-frame-pose-evidence":correspondenceMode?"offline-final-map-correspondence":result.supportOnly?
    ...
    out<<",\"matched_feature_count\":"<<(evidence?result.matchedFeatures.size():0)
       <<",\"matched_features\":[";
    if(evidence) for(size_t i=0;i<result.matchedFeatures.size();++i) {
    ...
    out<<(evidenceMode?",\"evidence_T_world_camera\":":",\"T_world_camera\":");
    if(!evidence) out<<"null";
    else {out<<'[';for(int r=0;r<4;++r){if(r)out<<',';out<<'[';for(int c=0;c<4;++c){if(c)out<<',';out<<result.Twc.at<double>(r,c);}out<<']';}out<<']';}
    out<<(evidenceMode?",\"evidence_pose\":":",\"pose\":");
    if(!evidence)out<<"null";
    else {
...
int main(int argc,char** argv) {
    try {
        ...
        const bool evidenceMode=std::string(input["purpose"])=="final-frame-pose-evidence";
        ...
        for(auto node:input["queries"]) {
            ...
            if(query.frame<0||!std::isfinite(query.time)||query.time<0||
               (evidenceMode?std::abs(query.time-boundaryTime)>5.:
                (query.frame>=boundaryFrame||query.time>=boundaryTime||boundaryTime-query.time>5.)))
                throw std::runtime_error("query_outside_initial_prefix_5s");
            if(!correspondenceMode&&!evidenceMode&&!node["original_pose_valid"].empty()&&int(node["original_pose_valid"])!=0)
                throw std::runtime_error("refusing_existing_pose");
            if(evidenceMode) {
                if(node["original_pose_valid"].empty()||int(node["original_pose_valid"])!=1)
                    throw std::runtime_error("evidence_requires_existing_pose");
                query.supportOnly=true;
            }
        ...
        if((correspondenceMode||evidenceMode)&&!input["support_queries"].empty())
        ...
        for(KeyFrame* kf:keyframes) if(kf&&!kf->isBad()&&kf->GetMap()==map&&
            kf->mTimeStamp>=boundaryTime-(evidenceMode?2.:1e-6)&&kf->mTimeStamp<=boundaryTime+2.) {
        ...
        if((correspondenceMode||evidenceMode)&&masks.find(control.frame)==masks.end())
            throw std::runtime_error("anchor_exclusion_mask_missing");
        control.excluded=masks[control.frame];
        if(!evidenceMode&&control.frame<boundaryFrame)throw std::runtime_error("anchor_frame_precedes_boundary");
        ...
        queries.erase(std::remove_if(queries.begin(),queries.end(),[&](const Query& query){
            return !evidenceMode&&query.supportOnly&&query.frame>=control.frame;
        }),queries.end());
        const int lastFrame=evidenceMode?std::max(control.frame,queries.back().frame):control.frame;
        GrayVideoReader reader(video,ffmpeg,size,lastFrame);
        ...
        Result check;
        if(evidenceMode) {
            // Evidence queries may lie after the control keyframe. Do not
            // seek backwards on the sequential query decoder.
            GrayVideoReader controlReader(video,ffmpeg,size,control.frame);
            check=localize(control,controlReader,extractor,points,descriptors,pointIds,K,D,size);
        } else check=localize(control,reader,extractor,points,descriptors,pointIds,K,D,size);
        ...
        for(auto& result:results) {
            if(evidenceMode) {
                // Pyramid levels can yield coincident pixels. localize()
                // exports a deduplicated set: report that exact set's metrics,
                // not the pre-dedup RANSAC count/RMS. Consumers recheck these.
                std::vector<double> errors;std::vector<cv::Point2f> pixels;
                std::set<int> cells;double squared=0.;
                for(const auto& match:result.matchedFeatures) {
                    errors.push_back(match.error);squared+=match.error*match.error;
                    pixels.emplace_back(match.pixel);
                    const int x=int(match.pixel.x*4/size.width),y=int(match.pixel.y*3/size.height);
                    if(x>=0&&x<4&&y>=0&&y<3)cells.insert(y*4+x);
                }
                result.inliers=int(errors.size());result.cells=int(cells.size());
                result.ratio=result.matches?double(result.inliers)/result.matches:0.;
                result.rms=errors.empty()?-1.:std::sqrt(squared/errors.size());
                std::sort(errors.begin(),errors.end());
                result.p95=errors.empty()?-1.:errors[size_t(.95*(errors.size()-1))];
                std::vector<cv::Point2f> hull;if(pixels.size()>=3)cv::convexHull(pixels,hull);
                result.hull=hull.empty()?0.:cv::contourArea(hull)/double(size.area());
                result.candidate=result.candidate&&result.inliers>=30&&result.ratio>=.45&&result.cells>=5&&result.hull>=.06;
                result.accepted=false;
                result.connected=result.candidate&&anchorValid;
                result.reason=result.connected?"validated_final_map_pose_evidence":"pose_evidence_rejected";
            } else if(correspondenceMode) {
        ...
        out<<"{\"type\":\"metadata\",\"schema\":\""
           <<(evidenceMode?"readonly-final-frame-pose-evidence/v1":correspondenceMode?"readonly-final-map-correspondence/v1":"readonly-prefix-localization/v1")<<"\",\"map_id\":"<<mapId
        ...
        for(const auto& result:results)writeResult(out,result,mapId,map->mnRevision,effective,correspondenceMode,evidenceMode);
        return 0;
